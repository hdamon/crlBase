...
switch lower(type)
  case 'cell'
    if strcmpi(obj.centering,'cell')
      ...
      objOut = crlBase.typegridInSpace(newSizes,'origin',newOrigin,'directions',newDirections,'centering','cell');
    end
  case 'node'
    if strcmpi(obj.centering,'node')
      ...
      newOrigin = obj.origin - (0.5*obj.directions*ones(nDim,1))';
      newDirections = obj.directions;
      objOut = crlBase.typegridInSpace(newSizes,'origin',newOrigin,'directions',newDirections,'centering','node');
    end;
  otherwise
    error('Unknown centering type');
end
